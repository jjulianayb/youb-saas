...
function sessionFromAuth(body: AuthResponse, fallbackRefreshToken?: string): SupabaseSession {
...
export async function restoreSession(): Promise<SupabaseSession | null> { try { const raw = window.localStorage.getItem("youb-session"); if (!raw) return null; const saved = JSON.parse(raw) as SupabaseSession; const fresh = saved.expires_at && saved.expires_at > Math.floor(Date.now() / 1000) + 60 ? saved : await refreshSession(saved); window.localStorage.setItem("youb-session", JSON.stringify(fresh)); return fresh; } catch { window.localStorage.removeItem("youb-session"); window.localStorage.removeItem("youb-organization"); return null; } }
