...
function BeeAvatar({ avatarUrl, avatarLabel = "Bee" }: { avatarUrl?: string | null; avatarLabel?: string }) { return avatarUrl ? <img src={avatarUrl} alt="Avatar da Bee" className="h-10 w-10 rounded-2xl object-cover" /> : <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[var(--youb-navy)] text-sm font-bold text-white">{avatarLabel.slice(0, 2).toUpperCase()}</div>; }
...
export default function BeeShell({ context, avatarUrl, avatarLabel, onSubmit }: BeeShellProps) {
  ...
  return <div className="relative">
    {open && <section className="absolute bottom-16 right-0 z-10 w-[min(360px,calc(100vw-2rem))] overflow-hidden rounded-3xl border border-border bg-card text-card-foreground shadow-2xl">
      <header className="flex items-start justify-between bg-[var(--youb-navy)] p-5 text-white"><div className="flex gap-3"><BeeAvatar avatarUrl={avatarUrl} avatarLabel={avatarLabel} /><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-white/70">Bee · Assistente</p><h2 className="mt-1 text-lg font-bold">Estou aqui para ajudar</h2></div></div><button aria-label="Fechar Bee" className="rounded-full p-1 text-white/80 hover:bg-white/10" onClick={() => setOpen(false)}><X size={18} /></button></header>
