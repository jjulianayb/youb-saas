...
export default function RecordDrawer({ open, onClose, title, subtitle, badge, children, footer }: RecordDrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      />
      <aside
        className="absolute right-0 top-0 flex h-full w-full max-w-md flex-col bg-white shadow-2xl animate-[slideInRight_0.3s_ease-out]"
      >
        <style>{`
        ...
        {footer && <footer className="border-t border-slate-200 p-5">{footer}</footer>}
      </aside>
    </div>
  );
}
...
export function DetailSection({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="mt-5">
      <h3 className="mb-2 text-xs font-bold uppercase tracking-[0.12em] text-blue-600">{title}</h3>
      {children}
    </div>
  );
}
