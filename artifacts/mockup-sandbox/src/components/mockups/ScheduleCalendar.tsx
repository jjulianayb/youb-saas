  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-4">
        ...
        <span className="flex items-center gap-2 text-sm font-semibold text-slate-600">
          <span className="h-3 w-3 rounded-full bg-blue-500" /> Check-ins
        </span>
      </div>
      ...
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button className="rounded-xl border border-slate-200 px-3 py-2 text-sm font-bold text-slate-600 transition hover:border-blue-300 hover:bg-blue-50" onClick={goToPreviousMonth} type="button">‹</button>
          <button className="rounded-xl border border-slate-200 px-3 py-2 text-sm font-bold text-slate-600 transition hover:border-blue-300 hover:bg-blue-50" onClick={goToToday} type="button">Hoje</button>
          <button className="rounded-xl border border-slate-200 px-3 py-2 text-sm font-bold text-slate-600 transition hover:border-blue-300 hover:bg-blue-50" onClick={goToNextMonth} type="button">›</button>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
        ...
        <div className="grid grid-cols-7">
          {days.map((cell, index) => {
            if (!cell.date) return <div key={`empty-${index}`} className="min-h-[64px] border-b border-r border-slate-100 bg-slate-50/50 sm:min-h-[88px]" />;
            ...
            return (
              <button
                key={cell.date}
                className={`flex min-h-[64px] flex-col items-start gap-1 border-b border-r border-slate-100 p-1.5 text-left transition sm:min-h-[88px] sm:p-2 ${
                  isSelected ? "bg-blue-50 ring-2 ring-inset ring-blue-400" : cell.isToday ? "bg-blue-50/40" : "hover:bg-slate-50"
                }`}
                onClick={() => setSelectedDate(cell.date)}
                type="button"
              >
                <span className={`flex h-6 w-6 items-center justify-center rounded-full text-xs font-bold ${
                  cell.isToday ? "bg-[#1e3a6e] text-white" : "text-slate-700"
                }`}>{cell.dayNumber}</span>
                <div className="flex flex-wrap gap-1">
                  {hasAssessment && <span className="h-2.5 w-2.5 rounded-full bg-amber-400" />}
                  {hasCheckin && <span className="h-2.5 w-2.5 rounded-full bg-blue-500" />}
                </div>
                {dayEvents.length > 0 && (
            ...
            <div className="mt-4 space-y-3">
              {selectedEvents.map((event) => (
                <div
                  ...
                  className="flex cursor-pointer items-center gap-3 rounded-xl border border-slate-200 p-3 transition hover:border-blue-300 hover:shadow-md"
                  onClick={() => event.kind === "assessment" ? onAssessmentClick(event.id) : onCheckinClick(event.id)}
                >
                  <span className={`flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full text-white ${event.kind === "assessment" ? "bg-amber-400" : "bg-blue-500"}`}>
                  ...
                  <span className={`rounded-full px-2 py-1 text-[11px] font-bold ${event.kind === "assessment" ? "bg-amber-50 text-amber-700" : "bg-blue-50 text-blue-700"}`}>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-3">
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          ...
          <p className="mt-2 text-2xl font-extrabold text-[#1e3a6e]">{monthEvents.length}</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          ...
          <p className="mt-2 text-2xl font-extrabold text-blue-600">{monthEvents.filter((event) => event.kind === "checkin").length}</p>
        </div>
      </div>
    </div>
  );
