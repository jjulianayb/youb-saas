...
type Employee = { id: string; full_name: string; email?: string | null; auth_user_id?: string | null; area_id?: string | null; position_id?: string | null; seniority?: "junior" | "pleno" | "senior" | null; manager_employee_id?: string | null };
type ManagedRole = "diretoria" | "rh" | "gestor" | "colaborador";
type MembershipRole = ManagedRole | "admin_youb";
...
export default function Dashboard({ session, organization, onLogout }: DashboardProps) {
  ...
  const [membershipRoles, setMembershipRoles] = useState<Record<string, MembershipRole>>({});
  ...
  const [employeeRole, setEmployeeRole] = useState<"" | ManagedRole>("");
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setUserRole(role);
      const organizationMembershipRows = role === "admin_youb" || role === "rh" || role === "diretoria" ? await apiRequest<Array<{ user_id: string; role: MembershipRole }>>(session, `memberships?select=user_id,role&organization_id=eq.${org}&order=created_at`) : [];
      setMembershipRoles(Object.fromEntries(organizationMembershipRows.map((membership) => [membership.user_id, membership.role])));
      const ownEmployeeRows = await apiRequest<Employee[]>(session, `employees?select=id,full_name,email,auth_user_id,area_id,position_id,seniority,manager_employee_id&organization_id=eq.${org}&auth_user_id=eq.${user}&limit=2`);
      ...
      const employeeRows = await apiRequest<Employee[]>(session, `employees?select=id,full_name,email,auth_user_id,area_id,position_id,seniority,manager_employee_id&organization_id=eq.${org}&${employeeFilters}&order=full_name`);
  ...
  function cancelEditEmployee() {
    setEditingEmployee(null); setEmployeeName(""); setEmployeeEmail(""); setEmployeeArea(""); setEmployeePosition(""); setEmployeeSeniority(""); setEmployeeManager(""); setEmployeeRole("");
  }
  ...
  async function saveEmployee(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (editingEmployee) {
        ...
        const created = await apiRequest<Employee>(session, "rpc/create_employee_profile", { method: "POST", headers: { "Content-Type": "application/json", Prefer: "return=representation" }, body: JSON.stringify({ p_organization_id: organization.id, p_full_name: payload.full_name, p_email: payload.email, p_area_id: payload.area_id, p_position_id: payload.position_id, p_seniority: payload.seniority, p_manager_employee_id: payload.manager_employee_id, p_status: "active", p_correlation_id: creationCorrelationId }) });
        employeeId = Array.isArray(created) ? created[0]?.id ?? null : created.id;
      }
      if (employeeRole) {
        if (!payload.email) throw new Error("Informe o e-mail do usuário antes de atribuir um perfil.");
        if (!employeeId) throw new Error("Não foi possível identificar o colaborador criado para concluir o vínculo.");
        await apiRequest(session, "rpc/link_organization_user", { method: "POST", headers: { "Content-Type": "application/json", Prefer: "return=representation" }, body: JSON.stringify({ p_organization_id: organization.id, p_email: payload.email, p_role: employeeRole, p_employee_id: employeeId }) });
      }
      setNotice(employeeRole ? "Colaborador salvo e acesso vinculado. O usuário deve entrar com este e-mail para abrir o perfil atribuído." : editingEmployee ? "Dados do colaborador atualizados." : "Colaborador adicionado à equipe.");
  ...
  return (
    <main className="yb-app-shell">
      <header className="yb-app-topbar">
        <div className="yb-app-logo"><strong>youB</strong><span /><small>HUMAN INTELLIGENCE<br />PLATFORM</small></div>
        <nav aria-label="Navegação principal da youB">
          {visibleNavItems.slice(0, 6).map((item) => <button key={item.id} aria-current={view === item.id ? "page" : undefined} className={view === item.id ? "active" : ""} onClick={() => { resetMessages(); setView(item.id); }} type="button"><span>{item.icon}</span>{item.label}</button>)}
        </nav>
        <div className="yb-app-user"><BeeShell surface="light" context={{ userName: firstName, organizationName: organization.name, role: userRole ?? "authenticated", capabilities: ["read:authorized-context", "recommend:review-only"], employeeLinked: Boolean(authenticatedEmployeeId), screen: "intelligence-workspace" }} /><div><small>{organization.name}</small><strong>{roleLabel(userRole)}</strong></div><button onClick={onLogout} type="button">Sair</button></div>
      </header>
      <nav className="yb-app-mobile-nav" aria-label="Navegação móvel da youB">
        {visibleNavItems.map((item) => <button key={item.id} aria-current={view === item.id ? "page" : undefined} className={view === item.id ? "active" : ""} onClick={() => { resetMessages(); setView(item.id); }} type="button"><span>{item.icon}</span>{item.label}</button>)}
      </nav>
      <section className="yb-app-content">
        {view !== "overview" && <header className="yb-app-page-header"><div><p>INTELLIGENCE WORKSPACE · {organization.name}</p><h1>{visibleNavItems.find((item) => item.id === view)?.label ?? "youB"}</h1><span>Contexto autorizado para {roleLabel(userRole).toLowerCase()}.</span></div><div><strong>{firstName}</strong><small>Plano Essencial</small></div></header>}
          <div className={view === "overview" ? "yb-app-home" : "yb-app-module"}>{notice && <div className="mb-5 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">{notice}</div>}{error && <div className="mb-5 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">{error}</div>}
            {loading ? <div className="rounded-2xl border border-slate-200 bg-white p-10 text-center text-sm text-slate-500">Carregando os dados da empresa...</div> : <>
              ...
              {canManageStructure && view === "team" && <ModuleSection title="Equipe" description="Cadastre colaboradores, áreas e cargos para dar contexto aos ciclos e planos.">
                ...
                <div className="mt-6 space-y-3">{employees.length === 0 ? <EmptyState text="Ainda não há colaboradores cadastrados." /> : employees.map((employee) => <div className="flex flex-col justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-5 sm:flex-row sm:items-center" key={employee.id}><div><h3 className="font-bold">{employee.full_name}</h3><p className="mt-1 text-sm text-slate-500">{employee.email || "Sem e-mail"} · {areaNames.get(employee.area_id ?? "") ?? "Sem área"} · {positionNames.get(employee.position_id ?? "") ?? "Sem cargo"} · {seniorityLabel(employee.seniority)} · {employee.auth_user_id ? roleLabel(membershipRoles[employee.auth_user_id]) : "Sem acesso"}</p></div><div className="flex items-center gap-2"><StatusPill status="active" /><button className={secondaryButtonClassName} onClick={() => { setHistoryEmployee(employee.id); resetMessages(); setView("employee-history"); }} type="button">Ver histórico</button><button className={secondaryButtonClassName} onClick={() => startEditEmployee(employee)} type="button">Editar</button></div></div>)}</div>
