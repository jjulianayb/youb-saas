...
export default function Dashboard({ session, organization, onLogout }: DashboardProps) {
  ...
  function startEditEmployee(employee: Employee) {
    setEditingEmployee(employee.id); setEmployeeName(employee.full_name); setEmployeeEmail(employee.email ?? ""); setEmployeeArea(employee.area_id ?? ""); setEmployeePosition(employee.position_id ?? ""); setEmployeeSeniority(employee.seniority ?? ""); setEmployeeManager(employee.manager_employee_id ?? ""); setEmployeeRole(""); resetMessages();
  }
