...
function SearchBar({ value, onChange, placeholder }: { value: string; onChange: (value: string) => void; placeholder: string }) {
  return <div className="relative mb-4">
    ...
    <input className="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-9 pr-3 text-sm text-slate-900 outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100" value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} type="search" />
  </div>;
}
          ...
          <div className="mx-auto mt-8 max-w-6xl">{notice && <div className="mb-5 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">{notice}</div>}{error && <div className="mb-5 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">{error}</div>}
            {loading ? <div className="rounded-2xl border border-slate-200 bg-white p-10 text-center text-sm text-slate-500">Carregando os dados da empresa...</div> : <>
              ...
              {canManageStructure && view === "team" && <ModuleSection title="Equipe" description="Cadastre colaboradores, áreas e cargos para dar contexto aos ciclos e planos.">
                ...
                <SearchBar value={searchQuery} onChange={setSearchQuery} placeholder="Buscar por nome, e-mail, área ou cargo..." /><div className="mt-6 space-y-3">{filteredEmployees.length === 0 ? <EmptyState text={q ? "Nenhum colaborador encontrado." : "Ainda não há colaboradores cadastrados."} /> : filteredEmployees.map((employee) => <div className="flex flex-col justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-5 sm:flex-row sm:items-center" key={employee.id}><div><h3 className="font-bold">{employee.full_name}</h3><p className="mt-1 text-sm text-slate-500">{employee.email || "Sem e-mail"} · {areaNames.get(employee.area_id ?? "") ?? "Sem área"} · {positionNames.get(employee.position_id ?? "") ?? "Sem cargo"} · {seniorityLabel(employee.seniority)}</p></div><div className="flex items-center gap-2"><StatusPill status="active" /><button className={secondaryButtonClassName} onClick={() => { setHistoryEmployee(employee.id); resetMessages(); setView("employee-history"); }} type="button">Ver histórico</button><button className={secondaryButtonClassName} onClick={() => startEditEmployee(employee)} type="button">Editar</button></div></div>)}</div>
