...
export default function Dashboard({ session, organization, onLogout }: DashboardProps) {
  ...
  return (
    <main className="youb-dashboard-shell min-h-screen bg-[#f5f8fc] text-slate-900">
      <div className="flex min-h-screen flex-col lg:flex-row">
        <aside className="youb-dashboard-sidebar w-full bg-[#102654] p-5 text-white lg:w-64 lg:p-6">
        ...
        <section className="youb-dashboard-content flex-1 p-5 sm:p-8 lg:p-10">
          <header className="youb-dashboard-header mx-auto flex max-w-6xl flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-violet-600">Intelligence Workspace · {organization.name}</p><h1 className="mt-2 text-3xl font-extrabold tracking-tight">Olá, {firstName}.</h1><p className="mt-1 max-w-2xl text-sm leading-6 text-slate-500">O que merece sua atenção agora — com pessoas, desenvolvimento e contexto no mesmo lugar.</p></div><div className="flex items-center gap-3"><BeeShell surface="light" context={{ userName: firstName, organizationName: organization.name, role: userRole ?? "authenticated", capabilities: ["read:authorized-context", "recommend:review-only"], employeeLinked: Boolean(authenticatedEmployeeId), screen: "intelligence-workspace" }} /><div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm shadow-sm"><span className="text-slate-400">Perfil </span><strong className="text-[#30367a]">{roleLabel(userRole)}</strong><span className="mx-2 text-slate-300">·</span><span className="text-slate-400">Acesso </span><strong className="text-[#30367a]">Contexto autorizado</strong></div></div></header>
