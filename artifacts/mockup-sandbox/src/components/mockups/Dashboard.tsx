...
const buttonClassName = "rounded-xl bg-blue-500 px-4 py-2.5 text-sm font-bold text-white transition hover:bg-blue-600 disabled:cursor-not-allowed disabled:opacity-50";
...
export default function Dashboard({ session, organization, onLogout }: DashboardProps) {
  ...
  return (
    <main className="min-h-screen bg-[var(--youb-surface)] text-slate-900">
      <div className="flex min-h-screen flex-col lg:flex-row">
        <aside className="w-full bg-blue-800 p-5 text-white lg:w-64 lg:p-6">
          ...
          <div className="mx-auto mt-8 max-w-6xl">{notice && <div className="mb-5 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">{notice}</div>}{error && <div className="mb-5 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">{error}</div>}
            {loading ? <div className="rounded-2xl border border-slate-200 bg-white p-10 text-center text-sm text-slate-500">Carregando os dados da empresa...</div> : <>
              {view === "overview" && (userRole === "gestor"
                ? <GestorHomeBody data={{ team: employees, areas, positions, checkins, feedbacks, pdis }} loading={false} onOpenTeam={() => { resetMessages(); setView("employee-history"); }} />
...
function MetricCard({ label, value }: { label: string; value: string | number }) { return <div className="rounded-2xl border border-slate-200 bg-white p-5"><p className="text-sm text-slate-500">{label}</p><p className="mt-2 text-2xl font-extrabold text-blue-500">{value}</p></div>; }
