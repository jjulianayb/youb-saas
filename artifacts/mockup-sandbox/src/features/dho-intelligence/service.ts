...
export async function readDhoContext(context: DhoReadContext): Promise<DhoContextResult> {
  ...
  const feedbackFailed = evolutionLoads.length > 0 && evolutionLoads.every(({ result }) => result.status === "unavailable");
  const feedbackEmpty = evolution.length === 0 && !feedbackFailed;
  ...
  const assessmentLimitation = assessmentLoad.limitation ?? (scoresLoad.status === "unavailable" ? scoresLoad.limitation : undefined);
  const pdiLimitation = pdiLoad.limitation ?? [objectivesLoad, actionsLoad, checkinsLoad].find((result) => result.status === "unavailable")?.limitation;
  return { items, populationMode: population.mode, populationCount: population.employeeIds.length, sources: dhoSourceAvailability({ assessment: assessmentLoad.status, feedback_360: feedbackFailed ? "unavailable" : (feedbackEmpty ? "empty" : "available"), pdi: pdiLoad.status }, { assessment: assessmentLimitation ?? "", feedback_360: feedbackFailed ? "Fonte DHO indisponível ou sem autorização para esta leitura." : "", pdi: pdiLimitation ?? "" }) };
}
