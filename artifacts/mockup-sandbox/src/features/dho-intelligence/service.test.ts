...
import { buildDhoContextItems } from "./service";
...
test("DHO adapter preserves provenance and rejects no-context as evolution", () => {
  const items = buildDhoContextItems({
    ...
    evolution: [{ source_type: "peer", round_id: "round-a", competency_id: "competency-a", competency_name: "Comunicação", expected_level_snapshot: 4, position_id: "position-a", score: 3, previous_score: null, delta: null }],
  ...
  assert.ok(items.some((item) => item.kind === "FEEDBACK_360" && item.unknowns.length > 0));
