...
import { sourceStateFromError, sourceStateFromRows, type CommercialSourceState, type CommercialSourceStatusMap } from "../../lib/data-readiness";
import { readDhoContext, type DhoContextItem, type DhoReadContext, type DhoRole, type DhoSourceAvailability } from "../dho-intelligence/service";
import {
  readActions,
...
export type BeeRuntimeReadModel = { context: Pick<BeeRuntimeContext, "organizationId" | "userId" | "role" | "employeeId" | "purpose">; readings: readonly BeeReadingBundle[]; assessments: readonly BeeAssessmentNode[]; recommendations: readonly BeeRecommendationNode[]; decisions: readonly BeeDecisionNode[]; interventions: readonly BeeInterventionNode[]; actions: readonly BeeActionNode[]; outcomes: readonly BeeOutcomeNode[]; memory: readonly BeeMemoryRef[]; events: readonly BeeEventRef[]; dhoContext?: readonly DhoContextItem[]; dhoAvailability?: readonly DhoSourceAvailability[]; sourceStatus?: CommercialSourceStatusMap; attentionToday: readonly BeeAttentionItem[]; };
...
export const BEE_INTENTS = ["attention_today", "attention_brief", "pending_confirmations", "leader_today", "leader_week", "leader_evolution", "explain_reading", "explain_hypothesis", "explain_evidence", "explain_recommendation", "explain_decision", "list_unknowns", "list_open_readings", "list_actions", "explain_outcome"] as const;
...
export type BeeRuntimeSource = {
  signals: IntelligenceSignal[]; evidence: IntelligenceEvidence[]; recommendations: IntelligenceRecommendation[]; interventions: IntelligenceIntervention[]; actions: IntelligenceAction[]; outcomes: IntelligenceOutcome[]; beeActionRequests: unknown[]; knowledgeSources: KnowledgeSource[]; knowledgeDocuments: KnowledgeDocument[]; organizationalMemoryRelations: OrganizationalMemoryRelation[]; organizationalEvents: OrganizationalEvent[]; decisions: IntelligenceDecision[]; decisionInterventions: IntelligenceDecisionIntervention[]; decisionRevisions: IntelligenceDecisionRevision[]; organizationalReadings: IntelligenceOrganizationalReading[]; organizationalReadingSources: IntelligenceOrganizationalReadingSource[]; organizationalReadingHypotheses: IntelligenceOrganizationalReadingHypothesis[]; evidenceAssessments: IntelligenceEvidenceAssessment[]; assessmentEvidence: IntelligenceEvidenceAssessmentEvidence[]; recommendationReadings: IntelligenceRecommendationReading[]; recommendationAssessments: IntelligenceRecommendationAssessment[]; recommendationHypotheses: IntelligenceRecommendationHypothesis[]; recommendationEvidence: IntelligenceRecommendationEvidence[]; recommendationRevisions: IntelligenceRecommendationRevision[]; dhoContext?: DhoContextItem[]; dhoAvailability?: DhoSourceAvailability[]; sourceStatus?: CommercialSourceStatusMap;
};

const emptySource = (): BeeRuntimeSource => ({ signals: [], evidence: [], recommendations: [], interventions: [], actions: [], outcomes: [], beeActionRequests: [], knowledgeSources: [], knowledgeDocuments: [], organizationalMemoryRelations: [], organizationalEvents: [], decisions: [], decisionInterventions: [], decisionRevisions: [], organizationalReadings: [], organizationalReadingSources: [], organizationalReadingHypotheses: [], evidenceAssessments: [], assessmentEvidence: [], recommendationReadings: [], recommendationAssessments: [], recommendationHypotheses: [], recommendationEvidence: [], recommendationRevisions: [], dhoContext: [], dhoAvailability: [] });
const safe = async <T>(sourceStatus: Record<string, CommercialSourceState>, key: string, loader: () => Promise<T[]>): Promise<T[]> => { try { const data = await loader(); sourceStatus[key] = sourceStateFromRows(data); return data; } catch (error) { sourceStatus[key] = sourceStateFromError(error); return []; } };

export function buildBeeReadContexts(context: BeeRuntimeContext): { intelligence: IntelligenceReadContext; dho: DhoReadContext } {
  const intelligence: IntelligenceReadContext = { session: context.session, organizationId: context.organizationId, employeeId: context.role === "colaborador" ? context.employeeId : null };
  const dho: DhoReadContext = { session: context.session, organizationId: context.organizationId, employeeId: context.employeeId, role: context.role as DhoRole };
  return { intelligence, dho };
}

export async function loadBeeRuntimeSource(context: BeeRuntimeContext): Promise<BeeRuntimeSource> {
  const contexts = buildBeeReadContexts(context);
  const sourceStatus: Record<string, CommercialSourceState> = {};
  const [signals, evidence, recommendations, interventions, actions, outcomes, beeActionRequests, knowledgeSources, knowledgeDocuments, organizationalMemoryRelations, organizationalEvents, decisions, decisionInterventions, decisionRevisions, organizationalReadings, organizationalReadingSources, organizationalReadingHypotheses, evidenceAssessments, assessmentEvidence, recommendationReadings, recommendationAssessments, recommendationHypotheses, recommendationEvidence, recommendationRevisions] = await Promise.all([
    safe(sourceStatus, "signals", () => readSignals(contexts.intelligence)), safe(sourceStatus, "evidence", () => readEvidence(contexts.intelligence)), safe(sourceStatus, "recommendations", () => readRecommendations(contexts.intelligence)), safe(sourceStatus, "interventions", () => readInterventions(contexts.intelligence)), safe(sourceStatus, "actions", () => readActions(contexts.intelligence)), safe(sourceStatus, "outcomes", () => readOutcomes(contexts.intelligence)), safe(sourceStatus, "bee_action_requests", () => readBeeActionRequests(contexts.intelligence)), safe(sourceStatus, "knowledge_sources", () => readKnowledgeSources(contexts.intelligence)), safe(sourceStatus, "knowledge_documents", () => readKnowledgeDocuments(contexts.intelligence)), safe(sourceStatus, "organizational_memory_relations", () => readOrganizationalMemoryRelations(contexts.intelligence)), safe(sourceStatus, "organizational_events", () => readOrganizationalEvents(contexts.intelligence)), safe(sourceStatus, "decisions", () => readDecisions(contexts.intelligence)), safe(sourceStatus, "decision_interventions", () => readDecisionInterventions(contexts.intelligence)), safe(sourceStatus, "decision_revisions", () => readDecisionRevisions(contexts.intelligence)), safe(sourceStatus, "organizational_readings", () => readOrganizationalReadings(contexts.intelligence)), safe(sourceStatus, "organizational_reading_sources", () => readOrganizationalReadingSources(contexts.intelligence)), safe(sourceStatus, "organizational_reading_hypotheses", () => readOrganizationalReadingHypotheses(contexts.intelligence)), safe(sourceStatus, "evidence_assessments", () => readEvidenceAssessments(contexts.intelligence)), safe(sourceStatus, "assessment_evidence", () => readAssessmentEvidence(contexts.intelligence)), safe(sourceStatus, "recommendation_readings", () => readRecommendationReadings(contexts.intelligence)), safe(sourceStatus, "recommendation_assessments", () => readRecommendationAssessments(contexts.intelligence)), safe(sourceStatus, "recommendation_hypotheses", () => readRecommendationHypotheses(contexts.intelligence)), safe(sourceStatus, "recommendation_evidence", () => readRecommendationEvidence(contexts.intelligence)), safe(sourceStatus, "recommendation_revisions", () => readRecommendationRevisions(contexts.intelligence)),
  ]);
  let dho: Awaited<ReturnType<typeof readDhoContext>> = { items: [], sources: [], populationMode: "empty", populationCount: 0 };
  try { dho = await readDhoContext(contexts.dho); sourceStatus.dho_context = sourceStateFromRows(dho.items); } catch (error) { sourceStatus.dho_context = sourceStateFromError(error); }
  return { ...emptySource(), signals, evidence, recommendations, interventions, actions, outcomes, beeActionRequests, knowledgeSources, knowledgeDocuments, organizationalMemoryRelations, organizationalEvents, decisions, decisionInterventions, decisionRevisions, organizationalReadings, organizationalReadingSources, organizationalReadingHypotheses, evidenceAssessments, assessmentEvidence, recommendationReadings, recommendationAssessments, recommendationHypotheses, recommendationEvidence, recommendationRevisions, dhoContext: dho.items, dhoAvailability: dho.sources, sourceStatus };
}
...
export function composeBeeReadModel(context: BeeRuntimeContext, source: BeeRuntimeSource, now = new Date()): BeeRuntimeReadModel {
  ...
  const dhoContext = (source.dhoContext ?? []).filter((item) => item.organizationId === context.organizationId && (context.role !== "colaborador" || item.employeeId === context.employeeId));
  const model: BeeRuntimeReadModel = { context: { organizationId: context.organizationId, userId: context.userId, role: context.role, employeeId: context.employeeId, purpose: context.purpose }, readings, assessments, recommendations, decisions, interventions: interventionNodes, actions, outcomes, memory, events, dhoContext, dhoAvailability: source.dhoAvailability ?? [], sourceStatus: source.sourceStatus, attentionToday: [] };
...
export function routeBeeIntent(input: string): BeeIntent | null { const value = input.trim().toLowerCase().replace(/[?!.]/g, "").replace(/\s+/g, "_"); if ((BEE_INTENTS as readonly string[]).includes(value)) return value as BeeIntent; const aliases: Record<string, BeeIntent> = { "what_deserves_my_attention_today": "attention_today", "attention": "attention_brief", "confirmations": "pending_confirmations", "what_needs_confirmation": "pending_confirmations", "leader_today": "leader_today", "today": "leader_today", "leader_week": "leader_week", "week": "leader_week", "leader_evolution": "leader_evolution", "evolution": "leader_evolution", "explain_a_reading": "explain_reading", "open_readings": "list_open_readings", "unknowns": "list_unknowns", "actions": "list_actions", "explain_a_hypothesis": "explain_hypothesis", "explain_evidence": "explain_evidence", "explain_a_recommendation": "explain_recommendation", "explain_a_decision": "explain_decision", "explain_an_outcome": "explain_outcome" }; return aliases[value] ?? null; }
