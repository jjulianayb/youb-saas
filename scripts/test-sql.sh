...
"${psql_base[@]}" -f "$repo_root/supabase/tests/pdi_development_v1.sql" > /dev/null 2>>"$log_file"
echo "PDI_SQL_SUITE=PASS"
"${psql_base[@]}" -f "$repo_root/supabase/tests/dho_bee_intelligence_wiring_v1.sql" > /dev/null 2>>"$log_file"
echo "DHO_BEE_SQL_SUITE=PASS"
"${psql_base[@]}" -f "$repo_root/supabase/tests/data_architecture_p0_event_capture_v1.sql" > /dev/null 2>>"$log_file"
...
"${psql_base[@]}" -f "$repo_root/supabase/tests/ambient_intelligence_foundation_v1.sql" > /dev/null 2>>"$log_file"
echo "AMBIENT_INTELLIGENCE_FOUNDATION_SQL_SUITE=PASS"
"${psql_base[@]}" -f "$repo_root/supabase/tests/company_onboarding_membership_v1.sql" > /dev/null 2>>"$log_file"
echo "COMPANY_ONBOARDING_MEMBERSHIP_SQL_SUITE=PASS"
