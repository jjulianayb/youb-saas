...
insert into public.ambient_observations(id,organization_id,source_registry_id,observation_type,epistemic_kind,summary,observed_at,sensitivity,confidence,scope_type,scope_ref,provenance,structured_value,actor_user_id,created_by_user_id)
values ((select value from ctx where key='obs_system'),(select value from ctx where key='org_a'),(select value from ctx where key='source_a'),'checkin','system_record','Check-in registrado pelo sistema','2026-09-01','standard',1.0,'organization',(select value from ctx where key='org_a'),'{"record_id":"checkin-1"}','{"engagement":4}',null,'10000000-0000-0000-0000-000000000001'),
 ((select value from ctx where key='obs_inferred'),(select value from ctx where key='org_a'),(select value from ctx where key='source_a'),'work_pattern','machine_inferred','Possível concentração de decisões','2026-09-02','restricted',0.4,'organization',(select value from ctx where key='org_a'),'{"basis":["checkin-1"],"method":"rule-preview"}','{"candidate":"delegation"}',null,'10000000-0000-0000-0000-000000000001'),
 ((select value from ctx where key='obs_b'),(select value from ctx where key='org_b'),(select value from ctx where key='source_b'),'checkin','system_record','Tenant B record','2026-09-01','standard',1.0,'organization',(select value from ctx where key='org_b'),'{"record_id":"b-1"}','{}',null,'10000000-0000-0000-0000-000000000001');
